...
function headerSearch() {
  ...
  const findTip = (!f.category || !f.product) ? "Pick a category and a product, or use a Beat chip." : "Resolver uses State + County + Category + Product. One result. Not three cards.";
...
function pageResult() {
  ...
  const headTip = house ? "Empty seat, not a dead end. Referral is with EMS. No fake card. Next: Specialist accept." : "Active Primary. Same agent the referral is created for. Next: Specialist accept.";
  return `<main class="page"><div class="grid2"><div class="panel">
    ...
    <div class="walk"><a class="btn-outline" data-tip="Opens the agent view of this same referral." href="#/accept">Specialist accept</a>
...
function pageAccept() {
  const rec = state.last;
  return `<main class="page"><div class="panel"><p class="tag mock">Specialist view · mocked delivery</p><h1>Referral ${rec ? rec.id : ""}</h1>
    ${rec ? `<p>${rec.county}, ${rec.state} · ${rec.product}</p><p>Assigned to ${rec.dest.agent} / ${rec.dest.agency}. Email + SMS with this id (mocked).</p>
    <div class="walk"><button class="btn-navy" style="width:auto" data-tip="Starts the 24 / 48 / 72 clock." onclick="var r=state.last; if(r){r.status='accepted';state.log.unshift({t:new Date().toISOString(),ev:'referral.accepted',id:r.id}); setHint('Accepted. Clock started.');}render()">Accept</button>
    <button class="btn-outline" data-tip="Back to statewide agents in this category. Not a drop." onclick="var r=state.last; if(r){r.status='rejected';state.log.unshift({t:new Date().toISOString(),ev:'referral.rejected',id:r.id,note:'poor fit to state + category RR'}); setHint('Rejected. Goes to statewide round-robin.');}render()">Reject, poor fit</button></div>
    <p>Status now: <b>${rec.status}</b></p><p class="fine">24 / 48 / 72 window. At 72 hours or reject, statewide round-robin in this category.</p>` : `<p>Submit a referral first.</p>`}</div></main>`;
}
function pageA2A() {
  return `<main class="page"><div class="panel"><h1>Agent-to-agent referral</h1><p>Same record type. Source = agent. Credit stays with the referring agent. Destination follows the same resolver.</p>
    <button class="btn-navy" style="width:auto" data-tip="Agent-to-agent. Credit stays with the sender. Resolver still sends Individual Health to House." onclick="const f={state:'Minnesota',county:'Hennepin',category:'Health & Medical Insurance',product:'Health, Individual'}; const r=resolve(f); const id=nextId(); state.referrals.unshift({id,source:'agent-to-agent',referring:'RP-A0001',...f,result:r.kind,dest:r.seat,status:'assigned-house',name:'A2A sample'}); state.log.unshift({t:new Date().toISOString(),ev:'referral.created',id,source:'agent'}); state.last=state.referrals[0]; location.hash='#/result'; render();">Send a Health Individual referral Mike does not hold</button></div></main>`;
}
...
document.querySelector(".poc-bar").addEventListener("click", (e) => {
  ...
  if (!b) return;
  const go = { "1": () => { preset("house"); location.hash = "#/"; }, "2": () => { preset("ems"); location.hash = "#/"; }, "3": () => { location.hash = "#/accept"; }, "4": () => { location.hash = "#/a2a"; }, "5": () => { location.hash = "#/territory"; }, "6": () => { location.hash = "#/replace"; }, "7": () => { location.hash = "#/records"; }, "8": () => { location.hash = "#/admin"; }, "9": () => { location.hash = "#/measure"; } };
  go[b](); render();
});
